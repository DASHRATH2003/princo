...
import { sellerRegister, saveUserSession } from "../services/authService";
...
const SellerRegister = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (data.user?.role !== 'seller') {
        throw new Error('Registration failed to create seller role.');
      }
      ...
      setVerification(v => ({
        ...v,
        sellerName: data.user?.name || name,
      ...
      try {
        const vdata = await getSellerVerification();
        if (vdata?.data) {
          setVerification(v => ({
            ...v,
            sellerName: vdata.data.verification?.sellerName || v.sellerName,
            shopName: vdata.data.verification?.shopName || v.shopName,
            email: vdata.data.verification?.email || v.email,
            phone: vdata.data.verification?.phone || v.phone,
            status: (vdata.data.verificationStatus || 'pending').replace(/\b\w/g, c => c.toUpperCase()),
            registeredOn: vdata.data.registeredOn ? new Date(vdata.data.registeredOn).toISOString().split('T')[0] : v.registeredOn,
          }));
        }
      } catch (ignored) {}
    } catch (err) {
  ...
  const handleVerifySubmit = async (e) => {
    ...
    try {
      ...
      const resp = await submitSellerVerification(formData);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-8">
        ...
        <form onSubmit={handleVerifySubmit} className="mt-6 space-y-5">
          ...
          {verifySuccess && <div className="text-green-600 text-sm text-center">{verifySuccess}</div>}

...
const AddProductModal = ({ isOpen, onClose, onProductAdded }) => {
  ...
  const handleVideoChange = (e) => {
    ...
    if (file.size > maxBytes) {
      setErrors(prev => ({ ...prev, video: 'Video size must be 5MB or less' }));
      e.target.value = '';
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (missing.length || multi.length) {
      const msgParts = [];
      if (missing.length) msgParts.push(`Select images for these colors: ${missing.join(', ')}`);
      if (multi.length) msgParts.push(`Select only one image per color (currently multiple selected for: ${multi.join(', ')})`);
              ...
              <div className="mt-4">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                  <input
                    ...
                    id="video-upload"
                  />
                  <label htmlFor="video-upload" className="cursor-pointer flex flex-col items-center text-gray-500">
                    {formData.video ? (
                      ...
                      <div className="flex flex-col items-center">
                        <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 48 48">
